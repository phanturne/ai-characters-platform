...
export default function EditCharacterPage() {
  ...
  const loadCharacter = useCallback(async () => {
    try {
      ...
      const characterData = await getCharacterById(supabase, {
        id: characterId,
      });
      const tagsData = await getTagsByCharacterId(supabase, { characterId });
      ...
      setFormData({
        ...
        messageExample: characterData.message_example || '',
        ...
        visibility: characterData.visibility as 'public' | 'private',
        tagIds: tagsData.map((ct) => ct.tag_id),
      });
    } catch (error) {
  ...
  }, [characterId, loadCharacter, loadTags]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedCharacter = await updateCharacter(supabase, {
        id: characterId,
        updates: {
          ...
          message_example: formData.messageExample.trim() || null,
          creator_notes: formData.creatorNotes.trim() || null,
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Update your AI character&apos;s personality, backstory, and
          conversation style
        </p>
      </div>
